...
public:
   ...
   void rotate(string type, vector<vector<int>>& grid) {
		...
		if (canRotate(pivot_x, pivot_y)) {
			...
			try {
				if ((state == 1 && grid.at(grid_y).at(grid_x - 1) == 0 && grid.at(grid_y - 1).at(grid_x) == 0 && grid.at(grid_y - 1).at(grid_x + 1) == 0) ||
					(state == 2 && grid.at(grid_y + 1).at(grid_x) == 0 && grid.at(grid_y).at(grid_x + 1) == 0 && grid.at(grid_y + 1).at(grid_x + 1) == 0)) {
					if (state == 1) {
   ...
   void move(string type) {
	...
	bool canMoveDown(vector<vector<int>>& grid) {
		for (const auto& sprite : sprites) {
			...
			if (y == 715 || grid[(y - 50) / 35 + 1][(x - 225) / 35] != 0) {
				return false;
			}
		}
		return true;
