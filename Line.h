...
public:
	...
	void rotate(string type, vector<vector<int>>& grid) {
		...
		if (canRotate(pivot_x, pivot_y)) {
			...
			try {
				if ((state == 1 && grid.at(grid_y).at(grid_x - 2) == 0 && grid.at(grid_y).at(grid_x - 1) == 0 && grid.at(grid_y).at(grid_x + 1) == 0) ||
					(state == 2 && grid.at(grid_y - 2).at(grid_x) == 0 && grid.at(grid_y - 1).at(grid_x) == 0 && grid.at(grid_y + 1).at(grid_x) == 0)) {
					if (state == 1) {
